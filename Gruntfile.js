...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    hapi: {
      ...
      prod: {
        options: {
          server: require('path').resolve('./dist/server/index.js')
        }
      }
    },
    open: {
      server: {
        url: 'http://localhost:4663'
      }
    },
    watch: {
      injectJS: {
  ...
  grunt.registerTask('serve', function (target) {
    if (target === 'dist') {
      return grunt.task.run(['build', 'env:all', 'env:prod', 'hapi:prod', 'wait', 'open', 'express-keepalive']);
    }
